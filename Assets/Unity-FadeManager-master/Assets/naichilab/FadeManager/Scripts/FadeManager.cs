...
public class FadeManager : MonoBehaviour
{
    ...
    {
        StartCoroutine(TransScene(scene, interval, null)); // コールバックがない場合は null を渡す
    }

    // コールバックを受け取るオーバーロードを追加
    public void LoadScene(string scene, float interval, Action onComplete)
    {
        StartCoroutine(TransScene(scene, interval, onComplete));
    }
    ...
    /// <param name='scene'>シーン名</param>
    /// <param name='interval'>暗転にかかる時間(秒)</param>
    /// <param name='onComplete'>フェードアウト完了後に実行するコールバック</param>
    private IEnumerator TransScene(string scene, float interval, Action onComplete)
    {
        //だんだん暗く .
        this.isFading = true;
        float time = 0;
